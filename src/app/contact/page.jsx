...
import { FaFacebookMessenger, FaGithub, FaInstagram, FaLinkedin, FaWhatsapp } from "react-icons/fa";
const ContactPage = () => {
  const socials = [
 ...
 useEffect(() => {
  ...
  return (
    ...
    <div className="h-full flex flex-col lg:flex-row px-4  sm:px-8 md:px-12 lg:px-28 xl:px-48  ">
          ...
          <div className='flex justify-center items-center gap-4 py-4'>
           {
              socials.map((social, index) => (
                <Link href={social.link} key={index} className='bg-white text-black rounded-full  drop-shadow-md hover:drop-shadow-xl p-2' >{social.icon} </Link>
       ...
       <form ref={form} onSubmit={sendEmail} className='h-2/3 mb-4 sm:mb-0 lg:h-full lg:w-1/2 flex flex-col justify-center p-10 gap-6 sm:p-24 sm:gap-8 bg-red-50 rounded-2xl'>
  <div className='flex flex-col gap-4'>
    <label htmlFor="user_email">Valid Email Address :</label>
    <input type="email" id="user_email" name="user_email" ref={emailRef} className='bg-transparent border-b-2 border-b-black outline-none' required />
  </div>
  <div className='flex flex-col gap-4'>
    <label htmlFor="user_message">Your Message:</label>
    <input type="text" id="user_message" name="user_message" className='bg-transparent border-b-2 border-b-black outline-none py-4' required />
  </div>
  <span>Regards</span>
  <button type='submit' className='bg-indigo-400 rounded font-semibold text-white p-4'>Send</button>
