  ...
  return (
    <motion.div className='h-full' initial={{y:"-200vh"}} animate={{y:"0%"}} transition={{duration:1}} >
        <div className='h-[1200vh]' ref={animateRef} >
            <motion.div  className='w-full h-[calc(100vh-6rem)] relative flex justify-center items-center'>
                <motion.h1 style={{x:`${current.x}px`, y:`${current.y}px`}} className='text-8xl text-center overflow-hidden absolute from-purple-600 via-pink-600 cursor-pointer to-blue-600 bg-gradient-to-r bg-clip-text text-transparent'>My Works</motion.h1>
            </motion.div>
            <div className='sticky top-0 flex h-screen gap-4 items-center overflow-hidden' >
               <motion.div style={{x}}  className='flex'>
                ...
                {
                    works.map((work)=>(
                       <div key={work.id} className={`h-screen w-screen flex justify-center items-center  bg-gradient-to-r ${work.color}`}>
                                <div className=' flex flex-col gap-6 text-white '>
                                    <h1 className='text-xl font-bold md:text-4xl lg:text-6xl xl:text-8xl'>{work.title}</h1>
                                   <div className='relative w-80 h-56 md:w-96 md:h-64 lg:w-[500px] lg:h-[320px] rounded'>
                                    <Image src={work.imag} fill sizes="4xl" alt={work.title} className='rounded' />
                                   </div>
                                   <p className='  w-80 md:w-96 lg:w-[600px]  xl:w-[800px]'>{work.desc}</p>
                                   <Link href={work.link} target='_blank' passHref className='ring-1 w-fit ring-white rounded px-8 hover:bg-white hover:text-black hover:ring-white self-end'>Demo</Link>
                                </div>
                       </div>
                    ))
                }
            ...
            <div className='h-screen w-screen flex flex-col justify-center items-center text-center'>
                <h1 className='text-2xl sm:text-4xl flex flex-col gap-1'>Do You Have Project ?
                <span className='text-sm text-red-800'>If you want to hire me, click 0n white dot </span>
                </h1>
                <div className='relative'>
                    <motion.svg animate={{rotate:360}} transition={{duration:8, ease:'linear', repeat:Infinity}} viewBox='0 0 300 300' className='h-64 w-64 md:h-[400px] md:w-[400px]'>
                        <defs>
                            <path
                            id='circlePath'
                        ...
                        <text fill='#000'>
                            <textPath xlinkHref='#circlePath' className='text-lg drop-shadow-md uppercase'>
                            Front End Developer
                            </textPath>
                        </text>
                    </motion.svg>
                    <Link href="/contact" passHref className='w-16 h-16 md:w-28 md:h-28 absolute top-0 bottom-0 left-0 right-0 m-auto bg-black text-white rounded-full flex flex-col items-center justify-center'>Hire Me
